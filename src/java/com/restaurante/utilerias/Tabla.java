...
public class Tabla {
    ...
    private String abrirTabla() {
        ...
        String cab = "<table class=\"table " + striped + "\" id='" + estilo + "' style='width:" + ancho + ";" + alin + "'>";
        return cab;
    }
    ...
    private String EncbzdTabla() {
        ...
        String cab = "<thead><tr align=center>";
        for (i = 0; i < cabeceras.length; i++) {
            if (anchocolumnas == null) {
                cab += "<th style=\"color: black;\">" + cabec[i] + "</th>";
            } else {
        ...
        try {
            while (k < rst[0].length) {
                ...
                if (isEliminable()) {
                    ...
                    Tabla += "<td>"
                            + "<a class=\"btn btn-inverse-danger\" onclick=\"return confirm('¿Está seguro? Esta acción no se puede deshacer.')\" href='" + getPaginaEliminable() + (getPaginaEliminable().contains("?") ? "&" : "?") + "id=" + rst[0][k] + "'>"
                ...
                if (isModificable()) {
                    ...
                    Tabla += "<td><a class=\"btn btn-inverse-primary\" onclick=\"return confirm('¿Está seguro?')\" href='" + getPaginaModificable() + (getPaginaEliminable().contains("?") ? "&" : "?") + "id=" + rst[0][k] + "'>"
...
//           }
    ...
    private String pieTabla() {
        ...
        String cab = "<tfoot><tr><td style=\"font-size: 26px\" align=center colspan='" + (numeroColumnas + ncol) + "'>" + pie + "</td></tr></tfoot>";
        return cab;
    }
